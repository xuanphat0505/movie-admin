...
export default function ReportTable({ reports, onOpenDetail }: ReportTableProps) {
  return (
    <div className="bg-white border border-slate-200/80 rounded-2xl dark:bg-slate-900/40 dark:border-slate-900/60 overflow-hidden flex flex-col">
      {/* Giao diện Table cho Desktop và Tablet */}
      ...
      {/* Giao diện danh sách dạng Card cho Thiết bị di động (Mobile) */}
      <div className="block sm:hidden divide-y divide-slate-100/60 dark:divide-slate-800/60">
        {reports.length > 0 ? (
          reports.map((report) => (
            <div key={report.id} className="p-4 flex flex-col gap-3">
              {/* Header của card: Tên phim, mã lỗi & Badge loại sự cố */}
              <div className="flex justify-between items-start gap-2">
                <div>
                  <h4 className="font-bold text-slate-850 dark:text-white text-xs leading-snug">
                    {report.movieName}
                  </h4>
                ...
                <span
                  className={`inline-block px-2 py-0.5 rounded-full text-[9px] font-bold whitespace-nowrap shrink-0 ${
                    report.errorType === "link"
                      ? "bg-rose-500/10 text-rose-500"
                      : report.errorType === "lag"
                        ? "bg-amber-500/10 text-amber-500"
              ...
              {/* Thông tin chi tiết: Tập phim & Người báo cáo */}
              <div className="grid grid-cols-2 gap-2 text-xs py-0.5">
                <div>
                  <span className="text-slate-400 block text-[9px] uppercase font-semibold tracking-wider">
                    Tập phim
                  </span>
                ...
                <div>
                  ...
                  <span className="font-semibold text-slate-700 dark:text-slate-350 block truncate">
                    {report.reportedBy}
                  </span>
              ...
              {/* Footer của card: Trạng thái & Nút xem chi tiết */}
              <div className="flex items-center justify-between pt-2.5 border-t border-slate-100/50 dark:border-slate-800/40">
                ...
                <button
                  onClick={() => onOpenDetail(report)}
                  className="inline-flex items-center gap-1 text-[10px] font-bold text-slate-500 hover:text-[#ff8300] bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 px-2 py-1 rounded-lg transition-all cursor-pointer"
                >
                  <Eye size={10} />
                  <span>Chi tiết</span>
                </button>
